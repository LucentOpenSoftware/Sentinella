    ...
    <header className="flex-shrink-0 glass-topbar px-14 py-3">
      <div className="app-shell-width grid items-center gap-5" style={{ gridTemplateColumns: "auto minmax(0, 1fr) auto" }}>
        ...
        <div className="flex items-center gap-4">
          <button onClick={onRefresh} className="rounded-xl p-2.5 text-[rgb(var(--t3))]/35 hover:bg-[rgb(var(--raised))]/25 hover:text-[rgb(var(--t2))] cursor-pointer" title="Refresh" aria-label="Refresh dashboard">
            <RefreshCw size={15} />
          </button>
          <button onClick={onNotifications} className="rounded-xl p-2.5 text-[rgb(var(--t3))]/35 hover:bg-[rgb(var(--raised))]/25 hover:text-[rgb(var(--t2))] cursor-pointer" title="Notification history" aria-label="Notification history">
            <Bell size={15} />
          </button>
